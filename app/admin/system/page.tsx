...
import { systemCourses, systemCourseStats } from "@/lib/mock-data"
...
export default function SystemCoursePage() {
  ...
  const handleImport = () => {
    alert("导入体系课包（演示：请选择.zip格式的课程包文件）")
  }
  ...
  const stats = [
    { label: "体系课总数", value: systemCourseStats.total, icon: BookOpen, color: "text-blue-600 bg-blue-100" },
    { label: "未提交", value: systemCourseStats.draft, icon: Clock, color: "text-gray-600 bg-gray-100" },
    { label: "审批中", value: systemCourseStats.pending, icon: Loader2, color: "text-yellow-600 bg-yellow-100" },
    { label: "已驳回", value: systemCourseStats.rejected, icon: XCircle, color: "text-red-600 bg-red-100" },
    { label: "已发布", value: systemCourseStats.published, icon: CheckCircle2, color: "text-green-600 bg-green-100" },
  ]
  ...
  const scopeTabs = [
    { key: "mine" as const, label: "我的体系课" },
    { key: "shared" as const, label: "共建体系课" },
    { key: "public" as const, label: "公共库体系课" },
  ]
    ...
    <div className="space-y-6">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/learn/courses/system/${course.id}`} className="flex items-center cursor-pointer">
                          ...
                          <DropdownMenuItem onClick={() => alert(`邀请共建：${course.name}（演示）`)}>
      ...
      {viewMode === "group" && (
        <div className="space-y-6">
          ...
          {filtered.length === 0 && (
            <div className="text-center py-8 text-muted-foreground">暂无符合条件的体系课</div>
          )}
        </div>
      )}
    </div>
